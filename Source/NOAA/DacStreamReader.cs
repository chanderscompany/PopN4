...
namespace DACarter.NOAA
{
	...
	// Dac 22Jul2005 
	//	Modified MS StreamReader class to be able to keep track of file position.
	//
	...
	**
	...
	[Serializable()]
	public class DacStreamReader : TextReader {
		// Note StreamReader.Null is threadsafe.
		/// <include file='doc\StreamReader.uex' path='docs/doc[@for="StreamReader.Null"]/*' />
		public new static readonly DacStreamReader Null = new NullStreamReader();
		...
		private byte[] byteBuffer;	// this is the input buffer[1024] from the Stream.Read()
		private char[] charBuffer;	// this is the decoded (from byteBuffer) char array[1024]
		private byte[] _preamble;   // Encoding's preamble, which identifies this encoding.
		private int charPos;
		private int charLen;		// count of chars in current charBuffer
		// Record the number of valid bytes in the byteBuffer, for a few checks.
		private int byteLen;

								// added by DAC 21July05
								// The actual length (in the file)
								// of the line read by ReadLine()
								// Includes all CR and LF chars.
								// Applies to most recent ReadLine call.
		...
		public DacStreamReader(Stream stream, bool detectEncodingFromByteOrderMarks) 
			: this(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize) {
		}
        
		/// <include file='doc\StreamReader.uex' path='docs/doc[@for="StreamReader.StreamReader1"]/*' />
		public DacStreamReader(Stream stream, Encoding encoding) 
		...
		public DacStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
		...
		public DacStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize) {
		...
		public DacStreamReader(String path) 
		...
		public DacStreamReader(String path, bool detectEncodingFromByteOrderMarks) 
			: this(path, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize) {
		}
        
		/// <include file='doc\StreamReader.uex' path='docs/doc[@for="StreamReader.StreamReader5"]/*' />
		public DacStreamReader(String path, Encoding encoding) 
			: this(path, encoding, true, DefaultBufferSize) {
		}

		/// <include file='doc\StreamReader.uex' path='docs/doc[@for="StreamReader.StreamReader6"]/*' />
		public DacStreamReader(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks) 
			: this(path, encoding, detectEncodingFromByteOrderMarks, DefaultBufferSize) {
		}

		/// <include file='doc\StreamReader.uex' path='docs/doc[@for="StreamReader.StreamReader7"]/*' />
		public DacStreamReader(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize) {
		...
		public override int Read([In, Out] char[] buffer, int index, int count) {
			...
			while (count > 0) {
				...
				if (n > count) n = count;
				if (!readToUserBuffer) {
					// InternalBlockCopy(src, srcOffset, dest, destOffset, count)
		...
		private int ReadBuffer() {
			...
			do {
				...
				// GetChars(src array, index, count, dest array, index)
		...
		private class NullStreamReader : DacStreamReader {
