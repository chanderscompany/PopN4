...
namespace DACarter.NOAA.Hardware {
    ...
    ///     and include "using libusbK" here.
	/// </summary>
    ...
    public class AD9959EvalBd {
		...
		private EndPoint UsbReaderEP1, UsbReaderDDS;
		private EndPoint UsbWriterEP1, UsbWriterDDS;
		...
		//
		private const int _vendorID = 0x456;
		private const int _productID = 0xEE07;
        private const int _altInterface = 0;
        private const int _USBReadEPID = 0x01;   // endpoint ID to read USB registers = 0x81
        private const int _USBWriteEPID = 0x01;  // endpoint ID to write USB registers
        private const int _DDSReadEPID = 0x08;   // endpoint ID to read DDS registers = 0x88
        private const int _DDSWriteEPID = 0x04;  // endpoint ID to write DDS registers
        //private UsbK _USBReadEP, _USBWriteEP, _DDSReadEP, _DDSWriteEP; 
        ...
        */
		...
		private void InitUSB() {
            ...
            _usbDevice = new UsbDevice(VendorID, ProductID, _altInterface);

            if (_usbDevice == null) {
                throw new ApplicationException("Cannot create usbDevice for AD9959.");
            }

            // The following is from old LibUsb library:
            ...
            UsbReaderEP1 = _usbDevice.AddReadEndpoint(1);      // USB endpoint 1 to read USB chip registers
            UsbWriterEP1 = _usbDevice.AddWriteEndpoint(1);     // USB endpoint 1 to write USB chip registers
            UsbReaderDDS = _usbDevice.AddReadEndpoint(8);      // USB endpoint 8 to read DDS registers
            UsbWriterDDS = _usbDevice.AddWriteEndpoint(4);     // USB endpoint 4 to write DDS registers

            if ((UsbWriterEP1 != null) && (UsbWriterDDS != null)) {
                _deviceName = UsbWriterDDS.DeviceName;
            }
            else {
                throw new ApplicationException("Cannot create AD9959 USB endpoints.");
            }
		...
		public string DetectDDS() {
			...
			//
			// dac note (from old UsbLib library):
		...
		private bool SetPortAValue(byte value) {
            ...
            bool success = _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_PORTA, value);
            if (!success) {
                return false;
            }
            // debug check:
            short outVal = _usbDevice.ReadPortValue(UsbDevice.PORTCOMMAND.READ_PORTA);
            if (outVal == value) {
                return true;
            }
            else {
		...
		private bool SetPortBValue(byte value) {
            ...
            _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_PORTB, value);
            // debug check:
            short outVal = _usbDevice.ReadPortValue(UsbDevice.PORTCOMMAND.READ_PORTB);
            if (outVal == value) {
                return true;
            }
            else {
		...
		private bool SetPortDValue(byte value) {
            ...
            _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_PORTD, value);
            // debug check:
            short outVal = _usbDevice.ReadPortValue(UsbDevice.PORTCOMMAND.READ_PORTD);
            if (outVal == value) {
                return true;
            }
            else {
		...
		private bool SetCtlValues(byte value) {
            ...
            _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_CTRL, value);
            // debug check:
            short outVal = _usbDevice.ReadPortValue(UsbDevice.PORTCOMMAND.READ_CTRL);
            if (outVal == value) {
                return true;
            }
            else {
		...
		public void USBWritePortBuffVal(fx2GPIO IOPort /*, short IOPortVal */ ) {
			...
			switch (IOPort) {
				case fx2GPIO.fx2_PortA:
					...
					bool success = SetPortAValue((byte)fx2PortVals[(int)IOPort]);
                    if (!success) {
                        throw new ApplicationException("Failed to Set USB Port A");
                    }
		...
		public void SetRdBackMode(bool value) {
            ...
            _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_READBACKMODE, mode);
		...
		public void SetRdBackByteCnt(short value) {
            ...
            _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_READBACKCNT, value);
		...
		public bool SetAutoCSBMode(byte value) {
            ...
            bool success = _usbDevice.WritePortValue(UsbDevice.PORTCOMMAND.WRITE_AUTOCSB, value);
            return success;
		...
		private void USBSerialLoad1Bit(int RegAddress, string Data, object NoUpRegMap) {
			...
			if (mvarEnabled) {
				...
				if ((Data.Length % 8) == 0 && RegAddress <= 27) {
                    ...
                    //EndPoint UsbWriterDDS = _usbDevice.GetWriteEndpoint(4);

					// Do the serial transfer
					//result = clsEZUSBDev01.BulkXfer(clsEZUSB.EZUSB_ReadOrWrite.ezWrite, buffer, lDatalen);
					int nBytes;
					bool success = UsbWriterDDS.Write(buffer, out nBytes);
					if (!success) {
						string errMsg2;
                        errMsg2 = "Error Writing to DDS Board: USBSerialLoad1Bit";
		...
		private string USBSerialRead1Bit(int RegAddress, object NoUpRegMap) {
			...
			if ((RegAddress >= 0) && (RegAddress <= 27)) {
				...
				if (_firstCallToSerialRead) {
					//
					// dac notes (from old UsbLib library):
					...
					success = UsbWriterDDS.Write(buffer, out nBytes);
					// this read times out:
					success = UsbReaderDDS.Read(ReturnBuff, out nBytes);
					SetRdBackByteCnt(lBytes2Rd);
				...
				success = UsbWriterDDS.Write(buffer, out nBytes);
				if (!success) {
                    string errMsg = "Error Writing Instruction to Board: USBSerialRead1Bit";
					throw new ApplicationException(errMsg);
				}
				...
				success = UsbReaderDDS.Read(ReturnBuff, out nBytes);
				if (!success) {
                    string errMsg = "Error Readback register on DDS Board: USBSerialRead1Bit";
                    throw new ApplicationException(errMsg);
				}
