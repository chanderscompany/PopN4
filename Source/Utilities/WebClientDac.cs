...
{
	/// <summary>
	/// Summary description for WebClientDac.
	/// </summary>
	public class WebClientDac
	{
		private WebClientDac()
		{
		}

		/// <summary>
		/// Returns a web page as a string of text characters.
		/// </summary>
		/// <param name="sUrl">The URL of the Web page.</param>
		/// <returns></returns>
		public static string GetWebPageAsString(string sUrl) {
			string sPage;

			WebClient myClient;
			Stream stream;
			System.Text.Encoding encode;
			StreamReaderX readStream;

			try {
				myClient = new WebClient();
				stream = myClient.OpenRead(sUrl);
			}
			catch (Exception e) {
				throw new System.ApplicationException("Error opening webclient");
			}

			try {
				encode = System.Text.Encoding.GetEncoding("utf-8");
				//encode = System.Text.ASCIIEncoding.ASCII;
				//readStream = new StreamReader(stream, encode);
				readStream = new StreamReaderX(stream);
			}
			catch (Exception e) {
				stream.Close();
				throw new System.ApplicationException("Error reading webclient stream");
			}

			try {
				sPage = readStream.ReadToEnd();
			}
			catch (Exception e) {
				stream.Close();
				readStream.Close();
				throw new System.ApplicationException("Error reading webclient page");
			}
			/*
		...
		public static void DownloadFile(string sUrl, string localFileName)
		{
			WebClient myWebClient = new WebClient();
			myWebClient.DownloadFile(sUrl, localFileName);
			myWebClient.Dispose();
		}
...
// The following is the source code for the System.IO.StreamReader Class
//  There is to be a bug in the .NET Framework somewhere because
//	sometimes the Stream.Read( ) method returns a -1  -- 
//	-- it should be zero or a positve value --
//	It seems to happen at the EOF when a zero should be returned.
//  But -1 fails the EOF test so the Decoder.GetChars() method
//	tries to read -1 characters, which throws an exception
//
// So I modified the test below marked in 2 places: // *** DAC ***
//
// For this to compile, had to comment out some Debug and __Error lines
//	and Environment.GetResourceString() lines
//	Also had to change Buffer.InternalBlockCopy to Buffer.BlockCopy
...
namespace DACarter.Utilities {
	...
	**
	...
	// 
	[Serializable()]
	public class StreamReaderX : TextReader {
		// Note StreamReader.Null is threadsafe.
		public new static readonly StreamReaderX Null = new NullStreamReader();
		...
		public StreamReaderX(Stream stream, bool detectEncodingFromByteOrderMarks) 
		...
		public StreamReaderX(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
		...
		// 
		public StreamReaderX(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize) {
		...
		public StreamReaderX(String path) 
			: this(path, Encoding.UTF8, true, DefaultBufferSize) {
		}
 
		public StreamReaderX(String path, bool detectEncodingFromByteOrderMarks) 
			: this(path, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize) {
		}
         
		public StreamReaderX(String path, Encoding encoding) 
			: this(path, encoding, true, DefaultBufferSize) {
		}
 
		public StreamReaderX(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks) 
			: this(path, encoding, detectEncodingFromByteOrderMarks, DefaultBufferSize) {
		}
 
		public StreamReaderX(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize) {
		...
		private class NullStreamReader : StreamReaderX {
