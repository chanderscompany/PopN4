// Listing2.cs - A base class for cancellable operations
// (C) 2001-2002 I D Griffiths
// See MSDN Magazine, Feb 2003, Vol 18, No. 2
//
// This base class is designed to be used by lengthy operations that wish to
// support cancellation.
//
// DAC Notes [6/11/2003]
// Derive your worker class from base class AsyncOperation:
//   public sealed class DavesWorker : AsyncOperation {}
// Constructor of derived class must receive Control object (e.g. a Form) to which
//   all events will be passed.
//   For example: public DavesWorker(ISynchronizeInvoke isi, ...): base(isi)
//   Need: using System.ComponentModel;
// Derived class must override: protected override void DoWork() {}
//   DoWork() must return immediately if it detects CancelRequested == TRUE.
// 
// In your application (UI thread):
// After creating instance of derived class, set event handlers for base class events:
//    worker = new DavesWorker(this);
//    worker.Completed += new EventHandler(WorkFinished);
//    worker.Cancelled += new EventHandler(WorkCancelled);
//    worker.Failed += new System.Threading.ThreadExceptionEventHandler(WorkFailed);
//    worker.Start();
// 
//   where the handler functions are defined like this:
//     private void WorkFinished(object sender, EventArgs e) {};
//     private void WorkCancelled(object sender, EventArgs e) {};
//     private void WorkFailed(object sender, System.Threading.ThreadExceptionEventArgs e) {};
// 
//  To cancel worker thread call:
//    worker.Cancel();
//  Check if work done by testing    worker.IsDone
//  For successful completion check  worker.HasCompleted
// 
//  Derived worker class can define its own events, for example:
//     public delegate void BigEventHandler(string name);
//     public event BigEventHandler BigEvent;
//  And fire the event this way:
//     if (BigDeal) {OnBigDeal(name);}
//     private void OnBigDeal(string name) {
//       lock(this) {
//         FireAsync(BigEvent, name);
//       }
//     }
//  The application UI thread handles the events this way:
//    worker.BigEvent += new DavesWorker.BigEventHandler(OnBigEvent);
//    private void OnBigEvent(string name) {...}
// 
...
namespace DACarter.Utilities
{
    ...
    public abstract class AsyncOperation
    {
        ...
        /// been successfully canceled, or has failed with an internal
        /// exception.
        /// </summary>
        ...
        public bool IsDone
        {
            get
            {
                lock(this)
                {
					// dac [6/12/2003] modified so IsDone==true when thread is not running,
					// so we don't wait for thread to finish when it has not even been started.
                    return completeFlag || cancelAcknowledgedFlag || failedFlag || !isRunning;
                }
            }
        }
        ...
        private void InternalStart()
        {
            ...
            {
                DoWork();
				// The following moved from derived class to base class
				// DAC [6/11/2003]
				if (CancelRequested) {
					AcknowledgeCancel();
				}
        ...
        protected void FireAsync(Delegate dlg, params object[] pList)
        {
            if (dlg != null)
            {
				IAsyncResult ares = Target.BeginInvoke(dlg, pList);
				//ares.AsyncWaitHandle.WaitOne();
				//Target.Invoke(dlg, pList);
			}
        }
    }
	...
	/// <summary>
	/// Exception class to propagate cancel operations through nested calls
	/// (saves having to check CancelRequested all the time)
	/// </summary>
	public class CancelThreadException: ApplicationException {
		public CancelThreadException() { 
		}
		public CancelThreadException(string message)
			: base(message) {
		}
		public CancelThreadException(string message, Exception inner)
			: base(message, inner) {
		}
	}
