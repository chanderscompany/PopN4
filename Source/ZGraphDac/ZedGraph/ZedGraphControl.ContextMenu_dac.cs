...
namespace ZedGraph
{
	...
	#region ContextMenu
		...
		private void contextMenuStrip1_Opening( object sender, CancelEventArgs e )
		{
			...
			if ( _masterPane != null && menuStrip != null )
			{
				...
				if ( _isShowContextMenu )
				{
					...
					item.Tag = "redraw";
					item.Text = "Redraw";
					item.Click += new System.EventHandler(this.MenuClick_Redraw);
					menuStrip.Items.Add(item);
		...
		/// <summary>
		/// Handler for the "Redraw" context menu item.  Forces a repainting of the graph.
		/// DAC -- 15Jan2008
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		protected void MenuClick_Redraw(System.Object sender, System.EventArgs e) {
			OkToRedraw = true;
			Refresh();
			//ZedGraphControl_ReSize(null, null);
			return;
		}
		...
		*/
		...
		public void ZoomOut( GraphPane primaryPane )
		{
			if ( primaryPane != null && !primaryPane.ZoomStack.IsEmpty )
			{
				this.OkToRedraw = true;	//dac: to force redraw on unzoom
				ZoomState.StateType type = primaryPane.ZoomStack.Top.Type;
