...
//-------------
// This version merges Zedgraph v5.1.4 with dac redraw changes
//  dac 15Jan2008
//-------------
...
*/
	...
	/// <version> dac $Revision:1.0$ $Date:1/14/2010 12:29:13 PM$ </version>
	public partial class ZedGraphControl : UserControl
	{

		private Bitmap _bmp;		// added by dac; bitmap object to draw to
		public bool OkToRedraw = true;	// added by dac; flag to allow full redraw
		public bool UseBitmapCopy = false;	// added by dac;
		private SolidBrush _redrawBrush1 = new SolidBrush(Color.FromArgb(100, Color.Blue));
		private SolidBrush _redrawBrush2 = new SolidBrush(Color.FromArgb(100, Color.Red));
		...
		protected override void OnPaint( PaintEventArgs e )
		{
			lock ( this )
			{
				...
				if (OkToRedraw) {
					...
					// test added dac 2007-08-16
					_bmp = new Bitmap(this.Width, this.Height);
					Graphics bmpGraphics = Graphics.FromImage(_bmp);
					Graphics drawing;
					...
					base.OnPaint(e);
					/*
					SmoothingMode sModeSave = drawing.SmoothingMode;
					TextRenderingHint sHintSave = drawing.TextRenderingHint;
					if ( _isAntiAlias )
					{
					...
					*/
					...
					//base.OnPaint(e);
					try {
						// DAC: draw either directly or from bitmap copy
						_masterPane.Draw(drawing); 
					}
					catch {	}
					OkToRedraw = false;

					//drawing.SmoothingMode = sModeSave;
					//drawing.TextRenderingHint = sHintSave;
					...
					if ((UseBitmapCopy) && (_bmp != null)) {
						e.Graphics.DrawImageUnscaled(_bmp, 0, 0);
						//e.Graphics.DrawString("Bitmap Image", new Font("Helvetica", 30.0f), _redrawBrush1, new PointF(0.0f, 0.0f));
					}
					else {
						e.Graphics.DrawString("Redraw", new Font("Helvetica", 30.0f), _redrawBrush1, new PointF(0.0f, 0.0f));
						e.Graphics.DrawString("Redraw", new Font("Helvetica", 30.0f), _redrawBrush2, new PointF(50.0f, 50.0f));
					}
				}
