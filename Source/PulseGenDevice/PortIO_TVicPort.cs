...
namespace DACarter.NOAA.Hardware {
	...
	/// <summary>
	/// Class that handles port I/O by using TVicPort.dll,
	///		which appears to need
	///		/WinNT/System32/Drivers/TVicPort.sys .
	///		May need to install by running TVicPortInstall41.exe .
	///		See http://www.entechtaiwan.com/dev/port/index.shtm .
	///		Need to purchase license ($25) to distribute.
	/// </summary>
	class PortIO_TVicPort : IOPortBase {

		private bool _driverOpened;

		public PortIO_TVicPort() {
			OpenDriver();
		}
		...
		public override byte ReadPort8(ushort port) {
			Byte value;
			if (HardwareExists) {
				value = TVicPort.ReadPort((ushort)port);
			}
			else {
				value = 0xFF;
			}
		...
		public override void WritePort8(ushort port, byte value) {
			if (HardwareExists) {
				TVicPort.WritePort(port, value);
			}
		}
		public override ushort ReadPort16(ushort port) {
			ushort value;
			if (HardwareExists) {
				value = TVicPort.ReadPortW((ushort)port);
			}
			else {
				value = 0xFFFF;
			}
		...
		public override void WritePort16(ushort port, ushort value) {
			if (HardwareExists) {
				TVicPort.WritePortW(port, value);
			}
		}
		...
		public void OpenDriver() {
            try {
                ...
                _driverOpened = (TVicPort.IsDriverOpened() != 0);
            }
            catch {
                _driverOpened = false;
            }
            if (_driverOpened) {
                HardwareExists = true;
                //throw new ApplicationException("Cannot open TVicPort Driver");
            }
            else {
		...
		unsafe private class TVicPort {
			...
			public static extern void* MapPhysToLinear(uint PhysAddr, uint Len);
