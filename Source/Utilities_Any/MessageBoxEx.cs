...
namespace DACarter.Utilities 
{
	/// <summary>
	/// MessageBoxEx class
	///		stolen from RodgerB http://www.codeproject.com/cs/miscctrl/CsMsgBoxTimeOut.asp
	///		MessageBox with timeout.
	///		Same format as the overloads for System.Windows.Forms.Messagebox 
	///		but all with an additional argument, which is timeout value in millisec.
	///		Without a button click from user, Message Box will disappear in timeout msec.
	/// </summary>
	public class MessageBoxEx
	{
		...
		////////////////////////////////////////////////////////////////
		//
		//	Attempt to add async capability to MessageBoxEx
		//
		////////////////////////////////////////////////////////////////
		...
		public static DialogResult ShowAsync(string text, uint timeout) {

			ThreadArgs args = new ThreadArgs();
			args.text = text;
			args.timeOut = timeout;
			args.caption = "";
			args.buttons = MessageBoxButtons.OK;
			args.icon = MessageBoxIcon.None;

			SimpleWorkerThread messageThread = new SimpleWorkerThread();
			messageThread.SetWorkerMethod(dummy);
			messageThread.Go(args);
			return DialogResult.OK;

		}

		public static DialogResult ShowAsync(string text, string caption, uint timeout) {
		...
		public static DialogResult ShowAsync(string text, string caption, MessageBoxButtons buttons, MessageBoxIcon icon, uint timeout) {
		...
		private static object dummy(object arg) {
			DialogResult rr = DialogResult.OK;
			if (arg is ThreadArgs) {
				ThreadArgs args = (ThreadArgs)arg;
				rr = MessageBoxEx.Show(args.text,
										args.caption,
										args.buttons,
										args.icon,
										args.timeOut);
		...
		////////////////////////////////////////////////////////////////
		//
		//	END -- Attempt to add async capability to MessageBoxEx
		//
		////////////////////////////////////////////////////////////////
		...
		private static bool Setup(string caption, uint uTimeout)
		{
			if (hHook != IntPtr.Zero) {
				//throw new NotSupportedException("multiple calls are not supported");
				Thread.Sleep((int)uTimeout + 100);
				if (hHook != IntPtr.Zero) {
					return false;
				}
			}
			...
			int threadID1 = System.Threading.Thread.CurrentThread.ManagedThreadId;
			int threadID2 = AppDomain.GetCurrentThreadId();
			uint threadID3 = GetCurrentThreadId();
