...
namespace DACarter.Utilities
{
	...
	public class CommPortWorkerThread2 : AsyncOperation {
		...
		*/
		...
		// helper function
		public static void SetUpCommPort(RS232 commPort,
									string portname,
									int baudrate,
									int databits,
									double stopbits,
			...
			*/
			...
			commPort.BaudRate = (RS232.BaudRates)Enum.Parse(typeof(RS232.BaudRates),baudName);
			...
			commPort.DataSize = (RS232.DataSizes)Enum.Parse(typeof(RS232.DataSizes),databitsName);
			...
			if (stopbits < 1.2) {
				...
				commPort.StopBit = RS232.StopBits.Bit2;
			}
			else {
				commPort.StopBit = RS232.StopBits.Bit1_5;
			}
			...
			if (parity[0] == 'N') {
				...
				commPort.Parity = RS232.Parities.Space;
			}
			else if (parity[0] == 'M') {
				commPort.Parity = RS232.Parities.Mark;
			}
			else if (parity[0] == 'E') {
				commPort.Parity = RS232.Parities.Even;
			}
			else if (parity[0] == 'O') {
				commPort.Parity = RS232.Parities.Odd;
			}
			else {
				commPort.Parity = RS232.Parities.None;
			}
